...
canvas = Canvas()
transform(point) = translate(scale(point, 5inch.value), 200)

textColor = RGBA(0.8, 0.7, 0.8, 0.9)
for (i, n) in enumerate(plot.nodes)
    rect = ((n.center .- n.radius)..., (n.center .+ n.radius)...)
    node = nodes[i]
    textbox = TextBox(text=String(node.id), rect=transform(rect), color=textColor)
    put!(canvas, textbox)
end
...
for points in plot.lines
    line = Line(points=transform.(points), thickness=thickness, color=strokeColor)
    put!(canvas, stroke(line))
end
for points in plot.arrows
    arrow = Polyline(points=transform.(points), thickness=thickness, color=strokeColor)
